...
import { Pattern, Folder, PatternVersion } from './pattern-store';

// Base directory for all files and folders
const ROOT_DIR = path.join(process.cwd(), 'patterns');
...
// File paths
const PATTERNS_FILE = path.join(ROOT_DIR, 'patterns.json');
...
// Get the file path for a pattern's content
const getPatternFilePath = (patternId: string, patternName: string) => {
  const sanitizedName = patternName.replace(/[^a-z0-9]/gi, '_').toLowerCase();
  return path.join(ROOT_DIR, `${sanitizedName}-${patternId}.md`);
};
...
// Save a pattern to the file system
export const savePattern = (pattern: Pattern) => {
  try {
    // Save pattern metadata
    const patterns = readJsonFile<Pattern[]>(PATTERNS_FILE, []);
    ...
    // Save pattern content to a separate file
    fs.writeFileSync(getPatternFilePath(pattern.id, pattern.name), pattern.content, 'utf8');
...
export const loadPatterns = (): Pattern[] => {
  try {
    return readJsonFile<Pattern[]>(PATTERNS_FILE, []);
  } catch (error) {
...
// Delete a pattern from the file system
export const deletePattern = (patternId: string) => {
  try {
    // Remove pattern metadata
    const patterns = readJsonFile<Pattern[]>(PATTERNS_FILE, []);
    ...
    writeJsonFile(PATTERNS_FILE, updatedPatterns);
    
    // Remove pattern file
    const filePath = getPatternFilePath(patternId, pattern.name);
    if (fs.existsSync(filePath)) {
...
export const deleteFolder = (folderId: string) => {
  try {
    ...
    // Remove folder directory (but not the patterns inside)
