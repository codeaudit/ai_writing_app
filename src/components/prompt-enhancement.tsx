...
import { z } from "zod";
import { enhancePrompt, savePromptTemplate } from "@/lib/prompt-enhancement";
...
export function PromptEnhancementButtons({ prompt, onPromptUpdate }: PromptEnhancementProps) {
  ...
  const onSaveTemplate = async (values: SaveTemplateFormValues) => {
    try {
      await savePromptTemplate(values.name, prompt, values.category || "General");
  ...
  // Return the buttons and the save dialog
  return (
    <>
      {/* Enhancement Button */}
      <Button
      ...
      <Dialog open={isSaveDialogOpen} onOpenChange={setIsSaveDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a reusable template from this prompt. It will be available in your templates library.
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
